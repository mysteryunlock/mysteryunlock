...
/**
 * Check whether the calling customer is already connected to a shop
 * identified by its connect code. Returns { connected: false } gracefully
 * when the caller has no customer record (e.g. a business owner testing the
 * page). Does NOT update last_visit — safe to call on every page load.
 */
export const checkShopConnectionFn = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator(z.object({ code: connectCodeSchema }))
  .handler(async ({ data, context }) => {
    let customerId: string | null = null;
    try {
      const result = await requireCustomer(context.userId);
    ...
    const { data: shop } = await supabaseAdmin
      ...
      .eq("is_active", true)
      .maybeSingle();
    if (!shop) return { connected: false as const };
    ...
    return { connected: !!conn as boolean };
  });

/**
 * Business owner scans a customer's personal QR code. Looks up the customer
 * by their connect code and returns their profile + membership info for the
 * calling owner's shop. Throws if the customer is not connected to any of the
 * caller's shops (prevents cross-shop data leakage).
 */
export const getMemberByCodeFn = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator(z.object({ customerCode: connectCodeSchema }))
  .handler(async ({ data, context }) => {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");

    // Resolve the customer by their personal connect code
    const { data: customer } = await supabaseAdmin
      .from("customers")
      .select("id, name, email, phone, connect_code")
      .eq("connect_code", data.customerCode)
      .maybeSingle();
    if (!customer) throw new Error("Member not found.");

    // Verify the calling user owns at least one shop this customer is linked to
    const { data: conn } = await supabaseAdmin
      .from("shop_customers")
      .select("status, last_visit, created_at, shops!inner(id, name, owner_user_id)")
      .eq("customer_id", customer.id)
      .eq("shops.owner_user_id", context.userId)
      .maybeSingle();

    if (!conn) throw new Error("This customer is not connected to your shop.");

    type ConnRow = {
      status: string;
      last_visit: string | null;
      created_at: string;
      shops: { id: string; name: string; owner_user_id: string };
    };
