...
export const LandingFooter = memo(function LandingFooter({ whatsappNumber, settings }: LandingFooterProps) {
  const businessName = settings?.business_name ?? "Mystery Unlock";
  const tagline = settings?.tagline ?? "Premium spin-to-win SaaS for boutique shops, salons, and cafes.";
  const email = settings?.email ?? "support@mysteryunlock.com";
  const whatsapp = settings?.whatsapp ?? whatsappNumber;
      ...
      <div className="max-w-7xl mx-auto px-5 md:px-8 py-14">
        <div className="grid md:grid-cols-[2fr_1fr_1fr_1.5fr] gap-8 md:gap-10">
          <div>
            <div className="flex items-center gap-2.5">
              <BrandMark size={36} />
              <span className="font-display font-bold text-lg" style={{ color: C.dark }}>{businessName}</span>
            </div>
            <p className="mt-4 text-sm max-w-xs" style={{ color: `${C.dark}99` }}>
              {tagline}
            </p>
            {address && (
              <p className="mt-3 text-xs" style={{ color: `${C.dark}80` }}>{address}</p>
            )}
            {(facebook || instagram || twitter) && (
              <div className="flex gap-3 mt-4">
                {facebook && (
                  <a href={facebook} target="_blank" rel="noopener noreferrer" className="hover:opacity-70 transition-opacity" aria-label="Facebook" style={{ color: C.dark }}>
                ...
                {instagram && (
                  <a href={instagram} target="_blank" rel="noopener noreferrer" className="hover:opacity-70 transition-opacity" aria-label="Instagram" style={{ color: C.dark }}>
                    <svg viewBox="0 0 24 24" className="w-4 h-4 fill-current"><path d="M16 11.37A4 4 0 1112.63 8 4 4 0 0116 11.37z"/><rect x="2" y="2" width="20" height="20" rx="5" ry="5" fill="none" stroke="currentColor" strokeWidth="2"/><path d="M17.5 6.5h.01" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>
                  </a>
                )}
                {twitter && (
                  <a href={twitter} target="_blank" rel="noopener noreferrer" className="hover:opacity-70 transition-opacity" aria-label="X / Twitter" style={{ color: C.dark }}>
        ...
        >
          <p className="text-xs" style={{ color: `${C.dark}80` }}>
            © {CURRENT_YEAR} {businessName}. All rights reserved.
          </p>
